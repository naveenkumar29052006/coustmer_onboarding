...
const SubserviceStep = ({ onBack, onComplete, setStepStatus }) => {
    ...
    const renderAccordion = (serviceNum) => {
        ...
        return (
            <div key={serviceNum} className={`border border-gray-200/50 rounded-lg overflow-hidden transition-all duration-300 shadow-sm hover:shadow-md mb-5 flex justify-center`} style={{ background: 'var(--Background-Primary, #FFFFFF)', borderRadius: '12px', width: '100%' }}>
                <button
                    className="w-full flex items-center focus:outline-none bg-transparent relative justify-center"
                    style={{
                        maxWidth: '600px',
                        width: '100%',
                        height: '80px',
                ...
                >
                    <div className="flex items-center flex-1 min-w-0 justify-center" style={{ height: '32px' }}>
                        <Circle />
                        <span
